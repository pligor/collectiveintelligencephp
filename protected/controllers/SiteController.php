...
class SiteController extends Controller
{
	...
	{
		$model = new MultitestForm();
		$form = $model->form;
		
		if( $form->submitted('submit', true) ) {
			//$form->validate();	//do not use this because of hidden values like (dummy_id)
			if( $model->save() ) {
				$this->redirect(array('site/edit','id'=>$model->models['Dummy']->id));
			}
		}
	...
	public function actionEdit($id) {
		$models['Dummy'] = Dummy::model()->findByPk($id);
		$iphones = $models['Dummy']->iphones;
		$models['Iphone'] = reset($iphones);
		
		$model = new MultitestForm($models);
		$form = $model->form;
	...
	public function actionContact()
	{
		...
		if(isset($_POST['ContactForm']))
		{
			...
			if($model->validate(/*array('subject')*/))
			{
